...
export default function WorkEnglishTestExam({ intro }) {
  ...
  const currentSection = getWorkEnglishTestSection(currentIndex)

  useEffect(() => {
    if (!import.meta.env.DEV) return
    const validation = validateWorkEnglishTestBank(workEnglishTestQuestions)
    if (!validation.valid) console.warn(`${intro.title} validation warnings:`, validation.errors)
  }, [intro.title])

  const start = () => {
    try {
      const nextAttempt = createWorkEnglishTestAttempt(workEnglishTestQuestions, attemptQuestions)
      ...
      if (import.meta.env.DEV) console.error('Work English Test attempt validation failed:', error.validationErrors ?? error)
