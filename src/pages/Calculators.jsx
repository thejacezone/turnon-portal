    ...
    <div className="portal-page calculators-page">
      ...
      <section className="calculator-tool-section page-section" aria-label="Calculadora de horas laborales">
        <CalculatorCard title="Calculadora de horas laborales" description="Ingresá tu salario mensual, agregá una o varias condiciones trabajadas y colocá la cantidad de horas. La herramienta estimará el valor de esas horas usando multiplicadores orientativos.">
          ...
          <LaborCalculatorRows lines={lines} calculatedLines={result.lines} errors={errors.lines || {}} onChange={updateLine} onRemove={(id) => setLines((current) => current.filter((line) => line.id !== id))} />
          <div className="calculator-total"><span>Total general</span><strong>{formatUSD(result.grandTotal)}</strong></div>
          <div className="calculator-actions">
            <button className="button" type="button" onClick={() => setLines((current) => [...current, newLine()])}>Agregar otra condición</button>
            <button className="button ghost dark-ghost" type="button" onClick={validate}>Revisar datos</button>
