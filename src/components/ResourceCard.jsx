  ...
  return (
    <Card className={`resource-card resource-card--${available ? 'available' : 'coming-soon'}`} eyebrow={resource.category} title={resource.title}>
      ...
      <div className="resource-card__footer">
        ...
        {available ? <Link className="button card-button resource-card__action" to={`/recursos/${resource.id}`}>{resource.actionLabel || 'Ver recurso'}</Link> : <span className="button card-button disabled resource-card__action" aria-disabled="true">Disponible pronto</span>}
      </div>
    </Card>
  )
}
