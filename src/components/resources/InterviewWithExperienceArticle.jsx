...
import '../../styles/interview-with-experience.css'
...
const articleSections = [
  ...
  { id: 'frases-utiles', label: 'Frases profesionales' },
  { id: 'metricas', label: 'Métricas sin inventar' },
...
function QuestionAccordion({ item, number }) {
  return (
    <details className="no-experience-question experience-question" id={item.id}>
      <summary>
...
export default function InterviewWithExperienceArticle({ article, resource }) {
  ...
  return (
    <div className="portal-page cv-guide-page no-experience-page experience-page">
      <header className="page-hero cv-guide-hero no-experience-hero experience-hero" aria-labelledby={`${resource.id}-article-title`}>
        {feathers.map((number) => (
          <img
            alt=""
            aria-hidden="true"
        ...
        <div className="page-hero-inner">
          ...
          <span className="page-hero-eyebrow">ENTREVISTAS / CON EXPERIENCIA</span>
        ...
        <article className="cv-guide-article">
          <section aria-labelledby="introduccion-title" className="cv-guide-section no-experience-intro" id="introduccion">
            ...
            <div className="cv-guide-copy">
              ...
              <div className="experience-comparison" aria-label="Comparación entre una respuesta débil y una mejor respuesta">
                <div className="is-weak"><strong>Débil</strong><p>{article.comparison.weak}</p></div>
                <div className="is-better"><strong>Mejor</strong><p>{article.comparison.better}</p></div>
              </div>
            </div>
          </section>
            ...
            <div className="cv-guide-copy">
              ...
              <div className="no-experience-levels">
                {article.howToUse.levels.map((level) => (
                  <div key={level.label}><strong>{level.label}</strong><p>{level.text}</p></div>
                ))}
              </div>
              <p className="cv-guide-paragraph">{article.howToUse.paragraphs[1]}</p>
              <aside className="no-experience-strategy experience-strategy" aria-labelledby="experience-strategy-title">
                <h3 id="experience-strategy-title">{article.strategy.title}</h3>
                <ol>{article.strategy.steps.map((step) => <li key={step}>{step}</li>)}</ol>
                <strong className="experience-strategy__label">Técnicas recomendadas</strong>
                <ul>{article.strategy.techniques.map((technique) => <li key={technique}>{technique}</li>)}</ul>
              </aside>
            </div>
          </section>
          ...
          <section aria-labelledby="errores-comunes-title" className="cv-guide-section" id="errores-comunes">
            <SectionHeading id="errores-comunes" number="03">Errores comunes al responder con experiencia</SectionHeading>
            <ol className="no-experience-errors">
              {article.commonErrors.map((error) => (
                <li key={error.title}><strong>{error.title}</strong><p>{error.explanation}</p></li>
              ))}
            </ol>
          </section>

          <section aria-labelledby="tecnicas-title" className="cv-guide-section" id="tecnicas">
            <SectionHeading id="tecnicas" number="04">Qué técnica usar para cada pregunta</SectionHeading>
            <div className="no-experience-table-wrap">
              <table>
                <thead><tr><th>Tipo de pregunta</th><th>Técnica recomendada</th></tr></thead>
                <tbody>{article.techniques.map(([question, technique]) => <tr key={question}><td>{question}</td><td>{technique}</td></tr>)}</tbody>
              </table>
            </div>
          </section>

          <section aria-labelledby="frases-utiles-title" className="cv-guide-section" id="frases-utiles">
            <SectionHeading id="frases-utiles" number="05">Frases útiles para sonar más profesional</SectionHeading>
            <ul className="no-experience-phrases">
              {article.professionalPhrases.map((phrase) => <li key={phrase}>{phrase}</li>)}
            </ul>
          </section>

          <section aria-labelledby="metricas-title" className="cv-guide-section" id="metricas">
            <SectionHeading id="metricas" number="06">{article.metrics.title}</SectionHeading>
            <p className="no-experience-section-lead experience-metrics-lead">{article.metrics.text}</p>
            <div className="experience-metrics-grid">
              {article.metrics.examples.map((example) => (
                <blockquote key={example.label}><strong>{example.label}</strong><p>{example.text}</p></blockquote>
              ))}
            </div>
          </section>

          <section aria-labelledby="cierre-title" className="cv-guide-section" id="cierre">
            <SectionHeading id="cierre" number="07">{article.conclusion.title}</SectionHeading>
            <div className="cv-guide-copy">
              {article.conclusion.paragraphs.map((paragraph) => <p className="cv-guide-paragraph" key={paragraph}>{paragraph}</p>)}
            </div>
          </section>

          <aside className="cv-guide-cta no-experience-cta" aria-labelledby="experience-cta-title">
            <span>SEGUÍ PRACTICANDO</span>
            <h2 id="experience-cta-title">{article.cta.title}</h2>
